...
def load_and_preprocess_data(path, batch_size=32, split_path=None):
    df = pd.read_csv(path)
    ...
    if split_path and os.path.exists(split_path):
        print(f"Lade Splits aus {split_path}")
        splits = np.load(split_path, allow_pickle=True).item()
        idx_train = splits["train"]
        idx_val = splits["val"]
        idx_test = splits["test"]
    else:
        print("Erzeuge neue Daten-Splits.")
        ...
        if split_path:
            np.save(split_path, {"train": idx_train, "val": idx_val, "test": idx_test})
            print(f"Splits gespeichert in {split_path}")

    X_train, y_train, point_ids_train = X[idx_train], y[idx_train], point_ids[idx_train]
    X_val, y_val, point_ids_val       = X[idx_val], y[idx_val], point_ids[idx_val]
    X_test, y_test, point_ids_test    = X[idx_test], y[idx_test], point_ids[idx_test]
    ...
    return train_loader, val_loader, test_loader, X_train.shape[1], weights, (idx_train, idx_val, idx_test)
